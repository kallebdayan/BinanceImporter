...
class CandleCollector:
    ...
    def collect_single_symbol(self, symbol: str, interval: str, 
        ...
        try:
            ...
            if start_time is None:
                last_time = self.get_last_candle_time(symbol, interval)
                if last_time:
                    start_time = last_time + self.INTERVAL_MS[interval]
                else:
                    # Primeira coleta - últimos 1000 candles
                    start_time = int((datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=30)).timestamp() * 1000)
    ...
    def fill_missing_data(self, symbol: str, interval: str, days_back: int = 30):
        ...
        start_time = int((datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=days_back)).timestamp() * 1000)
